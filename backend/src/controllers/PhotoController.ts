...
export class PhotoController {
  ...
  static async autoAddPhotoToEventActivities(photoId: number, eventName: string | null) {
    if (!eventName) return;
    try {
      const [activities]: any = await pool.query(`SELECT id FROM activities WHERE event_name = ? AND status != 'ENDED'`, [eventName]);
      for (const act of activities) {
        const actId = act.id;
        const [check]: any = await pool.query(`SELECT id FROM activity_photos WHERE activity_id = ? AND photo_id = ?`, [actId, photoId]);
        if (check.length === 0) {
          const [maxSortRows]: any = await pool.query(`SELECT MAX(sort_order) as maxSort FROM activity_photos WHERE activity_id = ?`, [actId]);
          const nextSort = (maxSortRows[0]?.maxSort || 0) + 1;
          await pool.query(`INSERT INTO activity_photos (activity_id, photo_id, sort_order) VALUES (?, ?, ?)`, [actId, photoId, nextSort]);
        }
      }
    } catch (err) {
    ...
    try {
      ...
      if (duplicate) {
        ...
        res.status(409).json({
          ...
          duplicate: {
            id: duplicate.id, thumbnail_url: duplicate.thumbnail_url ? `${BASE_URL}${duplicate.thumbnail_url}` : null,
            image_url: duplicate.image_url ? `${BASE_URL}${duplicate.image_url}` : null,
            faculty: duplicate.faculty || 'ไม่ระบุ', academic_year: duplicate.academic_year || 'ไม่ระบุ',
          },
        });
        return;
      }
      ...
      // ✅ บังคับเป็น "ไม่ระบุ" แทนที่จะเป็น null
      const finalFaculty = faculty?.trim() || 'ไม่ระบุ';
      const finalYear = academic_year?.trim() || 'ไม่ระบุ';

      const photo = await photoRepo.create({
        title: title.trim(), event_id: event_id ? parseInt(event_id) : null, event_date, description,
        image_url: imageUrl, thumbnail_url: thumbnailUrl, faculty: finalFaculty, academic_year: finalYear,
        file_hash: fileHash, user_id: userId, created_by: userId,
      });

      await PhotoController.autoAddPhotoToEventActivities(photo.id, photo.event_name);
      await historyService.log({
        actorId: userId, action: 'UPLOAD_PHOTO', targetType: 'PHOTO', targetId: photo.id,
        detail: { title: title.trim(), faculty: finalFaculty, academic_year: finalYear },
      });
  ...
  static async updatePhoto(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      if (!oldPhoto) { res.status(404).json({ success: false, message: `ไม่พบรูปภาพ ID ${id}` }); return; }
      ...
      const success = await photoRepo.update(id, {
        title, event_id: event_id !== undefined ? (event_id ? parseInt(event_id) : null) : undefined,
        event_date, description, image_url: newImageUrl, thumbnail_url: newThumbUrl,
        faculty: faculty !== undefined ? (faculty?.trim() || 'ไม่ระบุ') : undefined, // ✅ บังคับเป็นไม่ระบุ
        academic_year: academic_year !== undefined ? (academic_year?.trim() || 'ไม่ระบุ') : undefined,
        updated_by: userId,
      });
      ...
      if (success) {
        ...
        if (oldPhoto.event_id !== newEventId) {
          ...
          if (newEventId) {
            ...
            if (eventRows.length > 0) newEventName = eventRows[0].event_name;
          }
          await PhotoController.autoAddPhotoToEventActivities(id, newEventName);
        }
        await historyService.log({
          actorId: userId, action: 'UPDATE_PHOTO', targetType: 'PHOTO', targetId: id,
  ...
  static async deletePhoto(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      await historyService.log({
        actorId: userId, action: 'DELETE_PHOTO', targetType: 'PHOTO', targetId: photoId,
        detail: { deletedTitle: photo.title, faculty: photo.faculty || 'ไม่ระบุ' },
      });
    ...
    } catch (error: any) { sendError(res, error, 'ลบรูปภาพไม่สำเร็จ'); }
  }
  ...
  // ✅ รับ String ไปดึงตรงๆ
  static async getPhotosByEvent(req: Request, res: Response): Promise<void> {
    try {
      const eventId = parseInt(req.params.eventId as string, 10);
      const page    = Math.max(1, parseInt(req.query.page as string) || 1);
      const offset  = (page - 1) * PHOTO_PAGE_SIZE;
      if (isNaN(eventId)) { res.status(400).json({ success: false, message: 'Event ID ไม่ถูกต้อง' }); return; }

      const faculty = req.query.faculty as string | undefined;
      const year    = (req.query.year || req.query.academic_year) as string | undefined;
