...
export const ManageAdminPage: React.FC = () => {
  ...
  const [hardDeleteTarget, setHardDeleteTarget] = useState<UserEntry | null>(
    null,
  );
  ...
  const confirmHardDelete = async () => {
    ...
    try {
      ...
      await (AdminService as any).deleteUserPermanent(
        hardDeleteTarget.id,
        token,
      );
      ...
      setHardDeleteError(
        parseApiError(err, "ไม่สามารถลบได้: บัญชีนี้อาจมีข้อมูลกิจกรรมผูกอยู่"),
      );
    } finally {
  ...
  return (
    <Container className="py-5">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <p className="text-muted mb-0">
            ดู เปลี่ยนบทบาท ระงับ หรือลบสมาชิกในระบบ
          </p>
        </div>
            ...
            <tbody>
              {filteredUsers.length === 0 ? (
                ...
                filteredUsers.map((u, idx) => (
                  <tr
                    key={u.id}
                    style={{ opacity: u.role === "GUEST" ? 0.6 : 1 }}
                  >
                    ...
                    <td>
                      <Badge
                        bg={ROLE_BADGE[u.role] ?? "secondary"}
                    ...
                    <td className="text-muted small">
                      {/* ✅ ใช้ helper ที่จัดการเรื่อง UTC (+7) และปี พ.ศ. ให้เรียบร้อย */}
      ...
      >
        ...
        <Modal.Body className="py-4 px-4">
          <p className="text-muted small mb-3 text-center">
            กำลังแก้ไขบทบาทของ:{" "}
            <strong className="text-dark fs-6">{roleTarget?.username}</strong>
          </p>
        ...
        <Modal.Footer className="justify-content-center gap-3 border-0 pb-4 pt-0">
          ...
          <Button
            variant="warning"
            className="rounded-pill px-4 fw-bold text-dark"
      ...
      >
        ...
        >
          <Modal.Title className="fw-bold fs-5">
            🗑️ ระงับบัญชีผู้ใช้งาน
          </Modal.Title>
        </Modal.Header>
        <Modal.Body className="text-center py-4 px-4">
          <div className="mb-3" style={{ fontSize: "3.5rem", lineHeight: 1 }}>
            ⚠️
          </div>
          <h5 className="mb-3 fw-bold text-dark">
            ต้องการระงับบัญชี {deleteTarget?.username} ใช่หรือไม่?
          </h5>
          <p className="text-muted small mb-0">
            บัญชีจะถูกเปลี่ยนสถานะเป็น "ถูกระงับ"
            และไม่สามารถเข้าถึงระบบในฐานะสมาชิกได้
          </p>
          {deleteError && (
      ...
      <Modal
        ...
        onHide={() => {
          if (!restoring) setShowRestoreModal(false);
        }}
        centered
      >
        <Modal.Header
          closeButton={!restoring}
        ...
        <Modal.Body className="text-center py-4 px-4">
          ...
          <h5 className="mb-3 fw-bold text-dark">
            ต้องการยกเลิกการระงับบัญชี {restoreTarget?.username} ใช่หรือไม่?
          </h5>
          <p className="text-muted small mb-0">
            บัญชีจะถูกเปลี่ยนสถานะกลับเป็น "สมาชิก"
            และสามารถเข้าใช้งานระบบได้ตามปกติ
          </p>
          {restoreError && (
            <Alert variant="danger" className="mt-3 mb-0">
              {restoreError}
      ...
      <Modal
        ...
        onHide={() => {
          if (!hardDeleting) setShowHardDeleteModal(false);
        }}
        centered
      >
        <Modal.Header
          closeButton={!hardDeleting}
          className="bg-danger text-white border-0"
        >
          <Modal.Title className="fw-bold fs-5">
        ...
        <Modal.Body className="text-center py-4 px-4">
          ...
          <h5 className="mb-3 fw-bold text-danger">
            ต้องการลบบัญชี {hardDeleteTarget?.username} ถาวรใช่หรือไม่?
          </h5>
          <p className="text-muted small mb-0 fw-bold">
            การกระทำนี้ไม่สามารถกู้คืนได้
            ข้อมูลทั้งหมดที่เกี่ยวกับบัญชีนี้จะถูกลบออกจากฐานข้อมูล!
          </p>
          {hardDeleteError && (
